...
<table class="easyui-datagrid" id="bonusList" title="奖金列表" iconCls="icon-save" 
    ...
    <thead>
        <tr>
            ...
            <th data-options="field:'bonus',width:60,align:'right'">奖金金额</th>
...
<script>
    ...
    var toolbar = [{
        ...
        handler:function(){
        	...
        	if(mobiles.length == 0){
        		$.messager.alert('提示','未选中奖金条目!');
        		return ;
        	}
        	$.messager.confirm('确认','确定发放手机号为 '+mobiles+' 的奖金吗？',function(r){
        	    if (r){
        	    	var params = {"mobiles":mobiles};
                	$.post("/money/distributeBonus",params, function(data){
            			if(data.status == 200){
            				$.messager.alert('提示','发放奖金成功!',undefined,function(){
